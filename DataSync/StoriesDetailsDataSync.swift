...
class StoriesDetailsDataSync: NSObject {
    
    var is_running : Bool = false
    
    static let sharedInstance: StoriesDetailsDataSync = {
        let instance = StoriesDetailsDataSync()
        return instance
    }()
    ...
    func requestForSroriesDetails(id : String){
        
        //appDelegate.loadingView.alpha = 1.0
        //appDelegate.window?.bringSubview(toFront: (appDelegate.loadingView)!)
    
        let serverUrl = "\(baseUrl)/storydetails"
        let postString = "authentication=\(authkey)&story_id=\(id)"
        print("requestForSroriesDetails serverUrl: \(serverUrl) + &\(postString)")
    ...
    func processDataAsync(_ dataDic: NSDictionary?) -> Bool {
        ...
        DispatchQueue.main.async(execute: {
            ...
            if let topController = appDelegate.currentNaviCon?.visibleViewController {
                ...
                if (topController.isKind(of: ReaderViewController.self)){
                    (topController as! ReaderViewController).reloadData()
                }
