...
class StoriesListDataSync: NSObject {
    
    var is_running : Bool = false
 
    static let sharedInstance: StoriesListDataSync = {
        let instance = StoriesListDataSync()
        return instance
    }()
    ...
    func requestForStoriesList(){
        
        appDelegate.loadingView.alpha = 1.0
        appDelegate.window?.bringSubview(toFront: (appDelegate.loadingView)!)
        
        let serverUrl = "\(baseUrl)/storylists"
        let postString = "authentication=\(authkey)"
        print("requestForVehiclesList serverUrl: \(serverUrl) + &\(postString)")
        
        let request = NSMutableURLRequest(url: URL(string: serverUrl)!)
        //request.setValue("Bearer \((prefs.value(forKey:"api_token"))!)", forHTTPHeaderField: "Authorization")
                ...
                if (statusCode == 200) {
                    ...
                    do {
                        ...
                        if((resultDic?.value(forKey: "error")) != nil && (resultDic?.value(forKey: "error")) as! Int == 0){
                            //print("MFS Data:\(String(describing: resultDic))")
                            ...
                            DispatchQueue.main.async(execute: {
                                appDelegate.showAlert(message: resultDic?.value(forKey: "errorMsg") as! String, vc: appDelegate.currentNaviCon!.visibleViewController!)
                                    ...
                                    if itemDic[columnName!] != nil {
                                        if itemDic[columnName!] != nil && columnType?.caseInsensitiveCompare("BOOL") == .orderedSame {
                                            ...
                                            if itemDic[columnName!] != nil && !(((itemDic[columnName!] as AnyObject).isKind(of: NSNull.self))) {
                                                ...
                                                if (((itemDic[columnName!]) is NSArray)) {
                                                    //print("columnName : \(String(describing: itemDic![columnName!]))")
                                                    
                                                    if let objectData = try? JSONSerialization.data(withJSONObject: itemDic[columnName!] as Any, options: JSONSerialization.WritingOptions(rawValue: 0)) {
                                                        let objectString = String(data: objectData, encoding: .utf8)
                                                        //print("objectString : \(String(describing: objectString))")
                                                        
                                                        updateValues.append(self.getValue(value: objectString))
...
//                    (topController as! DashboardViewController).reloadData()
//                }
            }*/
        })
